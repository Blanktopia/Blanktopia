...
class CustomItemAction(config: ConfigurationSection) {
    ...
    fun run(key: String, player: Player, item: ItemStack) {
        ...
        consoleCommand?.let {
            val command = it.replace("%p", player.name)
        ...
        addPotionEffects?.let { player.addPermanentPotionEffects(key, it) }
        if (removePotionEffects) player.removePermanentPotionEffects(key)
        disguise?.let {
    ...
    fun run(key: String, player: Player, item: ItemStack, entity: Entity) {
        run(key, player, item)
        if (toggleItemFrameVisibility) { toggleItemFrameVisibility(player, item, entity) }
    }
}
...
fun toggleItemFrameVisibility(player: Player, item: ItemStack, entity: Entity) {
