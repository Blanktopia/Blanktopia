...
class MySQLStorage(private val hostname: String, private val port: Int, private val username: String, private val password: String, private val database: String, private val useSSL: Boolean): IStorage {
    private lateinit var conn: MySQL
    private var playerDatas = mutableMapOf<UUID, PlayerData>()

    override fun enable() {
        conn = MySQL(Blanktopia.INSTANCE.logger, hostname, port, username, password, database, useSSL)
        setup()
    }
    ...
    override fun save() {
        Blanktopia.INSTANCE.logger.info("Saving ${playerDatas.size} players' data.")
    ...
    override fun loadPlayer(uuid: UUID, callback: (PlayerData) -> Unit) {
        ...
        conn.executeQuery("SELECT has_spawned_dragon FROM players WHERE uuid = ?", uuid.toString()) {
            try {
                ...
                Blanktopia.INSTANCE.getLogger().warning("An error occured while trying to load a player from the database!")
                e.printStackTrace()
            }
        }
    }
